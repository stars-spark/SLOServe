...
评审依据 tighten-hold 敏感性扫描把默认值从 `8.0s` 改为 `4.0s`，并新增只把 hold 改为 `2.0s` 的
`adaptive-q-fast` 正式高负载对照臂。扫描显示：无 hold 时低负载发生 6 次截断；`2.0s` 与 `8.0s`
在低负载都为零截断、零非 L0，但高负载分别有 12 与 3 次截断，旧默认没有换来额外低负载收益，
却损失约 75% 的高负载动作；hold 达 `12.0s` 后虽仍进入非 L0，却一次都截不到。`4.0s` 在相同
CPU 模拟中保持低负载零截断，并保留 9 次高负载截断，且比 `2.0s` 留出更多迟滞余量，因此成为新
默认。正式矩阵随 fast 臂从 21 点增至 24 点。新默认的两个重跑产物为
`results/raw/week7-lowload-depth-probe/probe-default-hold-4s.json` 与
`results/raw/week7-highload-depth-probe/highload-default-hold-4s.json`；它们仍只是基于 expK-B 拟合的
确定性 CPU 模拟，不是真机结果。

此前实现修订保留 §5.3 的低负载零截断硬门，但修正了使该硬门按构造不可达的控制律。原始
...
仍可直接跳到自身证据已满足的最紧档。当时暂定的 `8.0s` 来自 episode 分离启发式：同一低负载 CPU 探针的
P50 为 4.611161s、P95/max 为 6.151278s；expK-B `rps=0.17` no-clip 的每 repetition 主导 `Q>=1` episode
P50 为 13.324348s、P95/max 为 43.151063s。该推导现已被敏感性数据取代；旧值下相同低负载序列保存在
...
预算。该阶段正式矩阵由 30 点缩减为 21 点；本次评审新增 fast 臂后调整为 24 点。本节引用的是
expK-B 既有原始数据的离线重建结果，不是 expL
的新实验结果或本方案提出的目标值。
...
`[0.25,0.75,1.5]`，墙钟 EWMA 时间常数 `6.0s`，连续高压收紧保持时间 `4.0s`，连续低压放松保持
时间 `30.0s`。三个收紧阈值全部落在
...
- 每个更紧档在 `P(t)` 达到自己的收紧阈值时独立开始计时；连续保持 `4.0s` 才具备进入该档的资格，
...
- 收紧仍快于放松，但不再是纯瞬时：4 秒在固定低负载 CPU trace 上过滤了会造成截断的 Poisson
  瞬态，30 秒放松 hold 防止控制动作自我熄火。代价是高负载首次收紧至少晚 4 秒，并在离散事件
  驱动下最晚到下一个更新才被观察，即
...
来自 30 秒放松记忆，而不是持续高压。最初据此要求收紧 hold 必须超过低负载最长 episode
6.151278 秒，并暂取 8 秒；§2.3.1 的闭环扫描证明这个解析式推导只是启发式，数据并不支持把它当成
必要条件：2 秒已经做到低负载零截断、零非 L0。很可能的原因是控制器只在队列事件上更新，低负载
事件稀疏，episode 内部常没有中间更新点；连续时间阈值最终被离散事件采样，不能只靠 episode 长度
解析推出动作次数。**这正是要扫描而不是解析推导的原因。**

高负载侧直接重建 expK-B 三 seed × 三 repetition 的 no-clip 等待区间：若只选每个 repetition 最长的
主导 `Q>=1` episode，范围为 10.422251–43.151063 秒，nearest-rank P50 为 13.324348 秒；但这一
“每轮取最大值”统计不能代表典型事件。按 §2.3.1 修正后的全事件口径，只有 9/213（4.2%）个
`Q>=1` episode 长达至少 8 秒；`Q>=2` 为 3/10，`Q>=3` 为 6/14。因此 8 秒只放行少数长事件，
是否仍产生足够的高负载控制驻留必须由闭环探针和正式 sweep 判断，不能从每轮最大值直接推出。
评审最终选择 4 秒默认：它在固定低负载 trace 上仍为零截断，高负载保留 9 次动作，在扫描响应尺度
上位于 2 秒 fast 臂与旧 8 秒默认之间，并比 2 秒留出更多迟滞余量；2 秒则作为正式对照交给真机直接
辨识“低负载纯净度 ↔ 高负载响应能力”。
...
高负载全请求口径的 213 个 `Q>=1` episode 中包含可立即 dispatch 所形成的几十微秒记录间隔；保留
这些原始间隔时 P95 为 6.070000 秒、max 为 43.151063 秒。此前的 177 个事件及 10.422251 秒 P95
来自只保留正式请求、排除每轮 4 个 warmup 的口径。默认 `adaptive_clip_relax_hold_s=30s` 高于全请求
实测 P95，但不覆盖最大 episode；它仍是防止短暂归零自我熄火的保守记忆，而不是保证覆盖每个极端
事件。若从 L3 开始，逐档重新计时使完全放松至少需要 90 秒。代价是拥塞真正结束后可能继续截断，
因此低负载确定性零截断门和真机吞吐等价区间必须共同约束它。

#### 2.3.1 Week-7 高负载 CPU 探针与 tighten-hold 敏感性

> **证据范围：以下新增结果全部是基于 expK-B no-clip 服务时间拟合
> `S=0.01494*n+0.0156`（R²=0.9998）的确定性 CPU 模拟，不是真机测量，不得与 expK-B 或未来
> expL 的 GPU 实测混同。** 原始输出为
> `results/raw/week7-highload-depth-probe/highload-default-hold.json` 和
> `results/raw/week7-highload-depth-probe/tighten-hold-sensitivity.json`；评审后 4 秒新默认的独立重跑为
> `results/raw/week7-lowload-depth-probe/probe-default-hold-4s.json` 和
> `results/raw/week7-highload-depth-probe/highload-default-hold-4s.json`。历史文件均保留且未覆盖。

探针使用 `rps=0.17`、realistic 重尾长度、`max_in_flight=4`、每 seed 三次相同确定性 repetition、
24 个正式请求加 4 个 warmup，以及 seed `20250825/11/202`。warmup 保留在控制器时间线和以下统计
中，这与 `run_benchmark` 的状态机输入一致。除扫描的 `tighten_hold_s` 外，阈值、EWMA `tau`、
放松 hold 和 caps 都从 `AdmissionControlConfig` 继承；探针沿用既有的 learned clipping 显式开关。
时间加权窗口为首次 enqueue 到最后一次 dispatch，因而在最后一次 dispatch 才进入的档位可有进入和
clip 事实、但驻留时间为零。

新默认 `tighten_hold_s=4.0` 下，闭环模拟的稳定深度分布如下。聚合不是 seed 等权平均，而是先合并
各 trace 的墙钟秒数再求比例。

| seed | 稳定深度时间加权分布 | 峰值 |
| --- | --- | ---: |
| 20250825 | Q0 68.23%，Q1 3.17%，Q2 3.23%，Q3 13.35%，Q4 4.97%，Q5 7.05% | 5 |
| 11 | Q0 89.48%，Q1 5.61%，Q2 4.77%，Q3 0.14% | 3 |
| 202 | Q0 89.50%，Q1 8.67%，Q2 1.83% | 2 |
| 三 seed 合计 | Q0 83.21%，Q1 5.98%，Q2 3.27%，Q3 3.99%，Q4 1.47%，Q5 2.08% | 5 |

各深度事件使用 nearest-rank P50/P95；单元格为 `count / P50 / P95 / max`（秒）：

| seed | Q>=1 事件 | Q>=2 事件 | Q>=3 事件 |
| --- | --- | --- | --- |
| 20250825 | 3 / 43.085 / 43.085 / 43.085 | 3 / 42.469 / 42.469 / 42.469 | 9 / 11.107 / 21.787 / 21.787 |
| 11 | 15 / 0.857 / 13.295 / 13.295 | 3 / 7.513 / 7.513 / 7.513 | 3 / 0.215 / 0.215 / 0.215 |
| 202 | 6 / 5.995 / 10.438 / 10.438 | 6 / 1.265 / 1.601 / 1.601 | 0 / — / — / — |
| 三 seed 合计 | 24 / 1.334 / 43.085 / 43.085 | 12 / 1.601 / 42.469 / 42.469 | 12 / 4.230 / 21.787 / 21.787 |

4 秒默认的响应仍集中在部分 seed。档位单元格为“驻留秒数 / 进入次数”；
“响应延迟”是每个正深度 episode 从首次正深度到首次收紧的 `count/P50/P95/max`（秒）。

| seed | 非 L0 时间 | 进入非 L0 | L1 | L2 | L3 | 最紧档 | `TIGHTEN_HOLD_RESET` | 响应延迟 | clip / 满足 OutputClipper 条件 |
| --- | ---: | ---: | ---: | ---: | ---: | --- | ---: | --- | ---: |
| 20250825 | 22.1732% | 3 | 0.000 / 0 | 86.381 / 3 | 0.000 / 0 | L2 | 6 | 3/12.459/12.459/12.459 | 3 / 3 |
| 11 | 2.7132% | 3 | 12.456 / 3 | 0.000 / 0 | 0.000 / 0 | L1 | 0 | 3/9.143/9.143/9.143 | 6 / 6 |
| 202 | 0.0000% | 0 | 0.000 / 0 | 0.000 / 0 | 0.000 / 0 | L0 | 3 | 0/—/—/— | 0 / 0 |
| 三 seed 合计 | 7.4971% | 6 | 12.456 / 3 | 86.381 / 3 | 0.000 / 0 | L2 | 9 | 6/9.143/12.459/12.459 | 9 / 9 |

评审参考数字来自真实 expK-B no-clip JSONL，而上两表是拟合服务时间且会因 clipping 改变后续服务
时长的闭环模拟，二者不能直接当成同一实验比较。为核对输入事实，探针另从三份 no-clip JSONL、全部
请求（含 warmup）重建事件：Q>=1 为 `213 / 0.0000557 / 6.070 / 43.151`，Q>=2 为
`10 / 7.503 / 42.521 / 42.521`，Q>=3 为 `14 / 4.178 / 21.832 / 21.832`。count、P95 和 max
与评审给出的取一位小数结果一致；Q>=1 P50 取一位小数为 0.0。唯一表面差异是 Q>=2 P50：本探针的
nearest-rank 取第 5 个值 7.503 秒，得到 7.5 秒；若取偶数样本的上中位数，第 6 个值为 7.560 秒，
得到评审的 7.6 秒。此前 §2.3 写的 177 个 Q>=1 事件过滤了 `sequence_id<4` 的 warmup；本次 213
个事件保留 warmup，正好与评审口径一致。

下表固定相同 seed、负载、请求和其余控制器配置，只扫描 `tighten_hold_s`。L1/L2/L3 为三 seed
合计驻留秒数；所有百分比仍按合计墙钟时间加权。

| tighten hold | 低负载 clip | 低负载非 L0 | 高负载非 L0 | 高负载 L1/L2/L3 驻留秒数 | 高负载最紧档 | 高负载 clip |
| ---: | ---: | ---: | ---: | --- | --- | ---: |
| 0s | 6 | 5.1155% | 13.9344% | 77.030 / 33.813 / 67.018 | L3 | 15 |
| 2s | 0 | 0.0000% | 5.3622% | 12.456 / 0.000 / 56.332 | L3 | 12 |
| 4s | 0 | 0.0000% | 7.4971% | 12.456 / 86.381 / 0.000 | L2 | 9 |
| 6s | 0 | 0.0000% | 7.6650% | 12.456 / 72.970 / 16.047 | L3 | 6 |
| 8s | 0 | 0.0000% | 5.9622% | 0.000 / 62.883 / 16.047 | L3 | 3 |
| 12s | 0 | 0.0000% | 3.6533% | 0.000 / 42.867 / 5.497 | L3 | 0 |
| 16s | 0 | 0.0000% | 3.6533% | 0.000 / 48.324 / 0.040 | L3 | 0 |

敏感性扫描形成三条正式结论：

1. **迟滞确实必要。** `hold=0` 时低负载已有 6 次截断、非 L0 驻留 5.1155%，不能回到瞬时收紧。
2. **旧默认 8 秒偏保守且没有换来低负载收益。** 2 秒在低负载已经与 8 秒完全相同，都是零截断、
   零非 L0；高负载却保留 12 次截断，而 8 秒仅 3 次，即 8 秒牺牲约 75% 的高负载动作，低负载收益
   为零。原先“必须超过低负载最长 episode 6.151278 秒”的推导只是启发式；控制器只在队列事件上
   更新、低负载事件稀疏且 episode 内部常无中间更新点所造成的离散化效应，是数据与解析直觉不一致
   的合理解释。这正是要扫描而不是解析推导的原因。
3. **`hold>=12s` 会把控制器做死。** 控制器仍进入非 L0，但 12 秒和 16 秒两档都一次截断也没有；
   若当初只拍一个更“稳妥”的值，expL 就会成为纯空结果。

评审据此把 4 秒定为推荐默认：固定低负载仍为零截断、零非 L0，高负载保住 9 次截断；它处在 2 秒
fast 与旧 8 秒响应尺度之间，并比 2 秒有更多迟滞余量。同时新增只把 hold 改为 2 秒的
`adaptive-q-fast` 真机对照。CPU 扫描负责暴露机制与风险，但不能替代正式 GPU 上对延迟—效用权衡的
测量；驻留比例也不随 hold 单调，因为 cap 会改变后续服务完成和队列轨迹。
...
admission:
  ...
  adaptive_clip_tighten_hold_s: 4.0
...
| `adaptive_clip_tighten_hold_s` | 有限非负浮点数 | `4.0` | 收紧前连续高压保持时间；由 0/2/4/6/8/12/16 秒 CPU 扫描选择，低负载零截断且高负载保留 9 次动作；2 秒作为 fast 对照 |
...
本方案提出 6 个正式 arm，统一使用 FCFS、`max_in_flight=4`、realistic heavy-tailed workload、`force_exact_output_tokens: true` 和与 expK-B 相同的 learned predictor 信息边界：

1. `no-clip`：无截断基线。
2. `fixed-512`：当前最优固定方案基线。
3. `adaptive-q-default`：推荐的队列深度压力，使用第 2 节默认阈值。
4. `adaptive-q-fast`：与 `adaptive-q-default` 完全相同，唯一差异是 `tighten_hold_s=2.0`，直接测量低负载纯净度与高负载响应能力之间的 hold 权衡。
5. `adaptive-q-conservative`：同一信号但按下表提高收紧门槛、延长放松证据，用于检验低负载效用保护与高负载反应迟滞的权衡。
6. `adaptive-rho-fixed-capacity`：按墙钟到达率 EWMA 除以显式容量常数的对照；容量来源记录为本机降频档约 0.230 req/s，不在线用完成速率回授，以避免 cap→容量估计→cap 的内生环。
...
| --- | --- | --- | --- | ---: | ---: | ---: | ---: |
| `adaptive-q-default` | `queue_depth_ewma` | `[1.0, 2.0, 3.0]` | `[0.25, 0.75, 1.5]` | `6.0s` | `4.0s` | `30.0s` | `null` |
| `adaptive-q-fast` | `queue_depth_ewma` | `[1.0, 2.0, 3.0]` | `[0.25, 0.75, 1.5]` | `6.0s` | `2.0s` | `30.0s` | `null` |
| `adaptive-q-conservative` | `queue_depth_ewma` | `[2.0, 3.0, 4.0]` | `[0.5, 1.5, 2.5]` | `12.0s` | `4.0s` | `45.0s` | `null` |
| `adaptive-rho-fixed-capacity` | `rho_hat` | `[0.70, 0.85, 1.00]` | `[0.60, 0.75, 0.90]` | `10.0s` | `4.0s` | `10.0s` | `0.230` |

四个 adaptive arm 都使用 `[1536, 1024, 512]`。除明确预注册的 `adaptive-q-fast=2.0s` 外，没有
arm 有意保留 8 秒 hold；conservative arm 的保守性仍由更高阈值、更长 EWMA 和更长 relax hold
提供，rho arm 则保持其替代信号与固定容量自变量，二者的 tighten hold 均同步为新默认 4 秒。
`rho_hat` arm 的 `tau` 用于按墙钟统计到达率；分母固定为配置中的容量，禁止在正式运行途中用受 cap
影响的完成率回写。
...
- `rps=0.17` 保留全部 6 arm × 3 seed = 18 点，因为 fixed-512、default/fast/conservative 三个
  queue-depth 参数组与 rho 对照都用于辨识高负载延迟—效用 Pareto、hold 权衡和控制机制。
- `rps=0.08` 只保留 `no-clip` 与推荐的 `adaptive-q-default`，即 2 arm × 3 seed = 6 点。低负载的
  ...
  与 rho arm 又不是最终“零效用代价”主张的对象，把它们各跑三次不会增强该主张。fast 臂也不进入
  低负载正式矩阵；因此它的低负载纯净度只有相同固定 trace 的 CPU 证据，没有真机证据，最终结论
  必须明确标注这一边界，不能把 default 臂的低负载 GPU 结果外推给 fast 臂。

因此正式矩阵从 30 点调整为 `18+6=24 points`。没有保留任何被删除的低负载 arm；它们仍全部保留在
高负载矩阵中，所以没有丢失 fixed cap 基线、阈值敏感性或替代信号对照。

| 负载 | arm | tighten hold | seed 数 | 点数 |
| --- | --- | ---: | ---: | ---: |
| `rps=0.08` | `no-clip` | 不适用（自适应关闭） | 3 | 3 |
| `rps=0.08` | `adaptive-q-default` | `4.0s` | 3 | 3 |
| `rps=0.17` | `no-clip` | 不适用（自适应关闭） | 3 | 3 |
| `rps=0.17` | `fixed-512` | 不适用（固定截断） | 3 | 3 |
| `rps=0.17` | `adaptive-q-default` | `4.0s` | 3 | 3 |
| `rps=0.17` | `adaptive-q-fast` | `2.0s` | 3 | 3 |
| `rps=0.17` | `adaptive-q-conservative` | `4.0s` | 3 | 3 |
| `rps=0.17` | `adaptive-rho-fixed-capacity` | `4.0s` | 3 | 3 |
| 合计 | 低负载 2 arm；高负载 6 arm | — | — | **24** |

按本轮预算输入重算：低负载每点约 16 分钟，6 点为约 96 分钟；高负载沿用已知每点约 7–8 分钟，
18 点为 126–144 分钟；纯运行合计 222–240 分钟，即 3 小时 42 分至 4 小时。原 30 点“含冷却
约 7.6 小时”与纯运行中值约 352.5 分钟之差约 103.5 分钟，折合每点约 3.45 分钟的启动、检查和冷却
开销；按 24 点线性折算约 82.8 分钟。修订后的总预算因此约 305–323 分钟，即 **5 小时 05 分至
5 小时 23 分（计划窗口按约 5.1–5.4 小时，保守预留 5.5 小时）**。这是从给定单点时长和原 7.6 小时
...
进入 GPU 阶段。硬门本身不修改；修复的是控制器的可达性条件。相同 trace 在旧 8 秒 hold 下的历史
`probe-after-tighten-hold.json` 与新 4 秒默认的 `probe-default-hold-4s.json` 都给出
`clip_applied_count=0`、`non_l0_time_fraction=0`；新文件的稳定深度分布仍为 Q0
`0.982851567818027`、Q1 `0.010994322003677516`、Q2 `0.006154110178295506`，与无 hold 的历史输入
一致。这说明门槛现在可达是因为瞬态没有在离散更新点上撑过持续时间判据，而不是换了更温和的随机
序列或掩盖了正深度。
...
由 GPU 证明，证据边界比把所有 arm 的低负载 GPU 点混在一起更清晰。

`adaptive-q-fast` 不进入低负载正式矩阵，因此上述 GPU 随机性门只验证 4 秒推荐臂，不能证明 2 秒
fast 臂的真机低负载纯净度。fast 臂在固定 `rps=0.08` CPU trace 上确为零截断、零非 L0，但这只是
基于 expK-B 服务时间拟合的确定性 CPU 证据。若高负载真机结果支持 fast，结论必须同时写明其低负载
边界尚未获得真机验证，不能把它改称为已经通过完整低负载零效用门的推荐方案。
...
7. **fast/default 的预注册判读**：若 2 秒 fast 臂在高负载上明显优于 4 秒 default 臂，而低负载 CPU
   证据仍保持零截断、零非 L0，则说明 4 秒默认过保守；结论同时必须注明 fast 没有低负载真机证据。
   反之，若 fast 的高负载优势不明显，则说明该 hold 权衡在本机、单模型、当前负载规模下不可辨识，
   不得宣称 4 秒或 2 秒具有已测出的性能优势。两种结果都如实报告，不事后增删门槛或对照臂。
8. **方向异常**：Poisson burst 中 `Q_inst` 增加而记录的 `P(t)` 或目标档位变松，说明稳定深度定义、
   更新时间或锁内顺序有 bug；不得继续跑完整 GPU sweep。

若自适应再次输给固定 cap，优先按以下假设定位：

- **控制太迟**：首批长请求已经占满 in-flight slot，排队形成后才有正 `Q`，或 4 秒默认（以及作为
  对照的 2 秒 fast）tighten hold 吃掉了可干预窗口；看“首次正深度→首次收紧”、`TIGHTEN_HOLD_*`
  与这些请求的 dispatch 时间。
- **阈值太高/平滑或收紧 hold 太慢**：高负载长期 L0/L1，收益保留率低；看档位驻留、
  `Q_inst/Q_bar` 越线记录，以及越阈 episode 是否在该 arm 的 hold 到期前结束。
...
产出：24 点非对称矩阵装配；按 seed/负载/arm 聚合；paired 区间、低负载吞吐等价门、收益保留率、
...
fixture 必须断言恰好 24 个唯一 label：高负载 6 arm × 3 seed = 18 点，低负载 2 arm × 3 seed =
...
产出：`results/raw/week7-expL/` 下 24 个完整点的请求级 JSONL/CSV、decision sidecar、GPU samples、
completeness report、环境 metadata、sweep CSV/JSON 和 `analysis.json`。每点之间执行既有冷却门；
热保护触发就中止并保留部分结果。

验收：24 个唯一 label 全部存在，且满足高负载 18 点、低负载 6 点的预注册矩阵；每个点正式请求数、
