...
## 2026-09-04 — Week 6 batch 2b：纠正 max_tokens 语义并恢复 expK-B

首个截断点出现“施加 cap 但实际截断率为零”，触发了对请求级事实的反查。六个旧 no-clip seed 中，
采样目标与实际输出的 Pearson 相关仅约 0.33，1080 个正式请求只有 202 个输出等于目标；模型通常在
`max_tokens` 前自然 `stop`。因此 `max_tokens` 是上限而非真实输出长度，先前 expK-A 的 `true/oracle`
说法不成立，其 0.74/0.65/0.51 只能作为目标上限代理排序结果，不能证明真实长度知识的收益。

修复增加默认关闭的 `realistic_length.force_exact_output_tokens`，随 envelope 经过 reclock 和 clipping
保留；开启时 HTTP backend 发送 `min_tokens == max_tokens == effective cap`。MockTransport 回归测试覆盖
裁剪后的两者同步，真实 vLLM 探针也确认普通 512 cap 在 423 token `stop`，强制定长则在 512 token
`length`。旧七点诊断数据移至 `week6-expK-B-invalid-cap-proxy/`，不删除、不纳入正式结论。

定长后平均请求远长于旧校准，rps=1.2 进入非稳态过载；该部分另存
`week6-expK-B-overloaded-partial-rps1.2/`。容量探针后把正式点设为 rps=0.17、四个 cap arm × 三 seed，
使比较可在稳态附近完成。当前 no-clip/1536/1024 共九点完整，512 三点正在断点续跑。M/G/1 仍只作
方向基线，因为实际系统是 max_in_flight=4 且 vLLM 连续批处理，不满足单 FCFS server 假设。

## 2026-09-04 — Week 6 batch 2c：expK-B 完成，第一个真正有效的干预

12 点全部跑完（no-clip / 1536 / 1024 / 512，各 3 seed，rps=0.17、FCFS、max_in_flight=4、强制定长）。
断点续跑期间机器已改为关闭睿频 + `balanced` 电源档，全程峰值 GPU ≤63 °C、CPU ≤76 °C，未触发
71/88 °C 硬闸。

结果是本项目第 3–6 周里第一个明确为正的干预：截断到 512 token 使平均排队等待从 2.78 s 降到
0.26 s（10.6×）、p99 从 15.69 s 降到 3.85 s（4.1×），交互类 SLO 达成率从 0.26 升到 0.73——比前三周
任何调度策略改动带来的交互收益都大。机制与 M/G/1 预期一致：拟合平均服务时间只降 1.75×，而
`E[S²]` 降 2.94×，等待降 10.6×，即队列响应的是方差而不是工作量。服务时间对输出长度近乎完美线性
（`S = 0.01494·n + 0.0156`，R²=0.9998，n=216），这才使"把长度分布换算成服务时间分布"成立。

代价必须同时报告：512 档有 43.1% 的请求被平均砍掉 1019 token，token 吞吐从 146.6 降到 93.3
（−36%）。延迟变好有一部分原因是系统干的活变少了，所以这是"效用换延迟"，不是调度改进。

M/G/1 基线需要一次不体面的修正才可用。字面单服务器模型在这里是空的：`max_in_flight=4` 时哪怕最好
的一档利用率也有 2.63，模型判定全部不稳定、拒绝给出预测，而实测队列明显处于稳态。分析改为把拟合
服务时间除以实测并发度，即把引擎建模成"快 4 倍的单服务器"。这**不是** M/G/c（P-K 公式没有精确的
M/G/c 形式），而且偏乐观，因为它允许单个长请求独占整机服务率。对照实测的表现正好暴露这个缺陷：
四档的方向和排序全对（归一化 1.00/0.81/0.37/0.19 对实测 1.00/0.86/0.24/0.09），但绝对值整体高估约
两倍，且低估了截断的收益。因此它只作定性趋势核对，本节没有任何结论依赖它的绝对秒数。

顺带修掉两个过期测试夹具：`test_sweep.py` 仍假设 expK-B 是 24 点六 seed（重新校准后已是 12 点三
seed），`test_expkb_analysis.py` 的 CSV 夹具缺 `max_in_flight` 列。并发度从 sweep 结果读取而非硬编码，
所以夹具补列是必要的，不是把断言放松。
